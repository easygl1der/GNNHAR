...
The notebook clones the repository branch \texttt{2026-06-01} from GitHub, runs the
full GNNHAR-IV pipeline with the repository's tracked Dow 30 input data, writes outputs to
\path{/content/gnnhar_outputs}, and uploads those outputs to Google Drive through the Drive
API with a service account. This avoids \texttt{drive.mount} and therefore avoids the repeated
interactive Google Drive mount prompt.

\begin{itemize}
\setlength{\itemsep}{0pt}
\item Colab GitHub secret: \texttt{GITHUB\_TOKEN}.
\item Colab Drive secret: \texttt{GDRIVE\_SERVICE\_ACCOUNT\_JSON}.
\item Colab output-folder secret: \texttt{GDRIVE\_OUTPUT\_FOLDER\_ID}; the configured
folder ID is reproduced below.
...
\item Drive upload access: the destination folder is shared with the configured service
account, whose email is reproduced below.
...
\begin{lstlisting}[caption={Configured service-account destination.}]
GDRIVE_OUTPUT_FOLDER_ID = '1jdw1_WlULw7SmUbA3r7PK7zzUBI15O5x'
...
\begin{lstlisting}[language=Python,caption={Upload local outputs to Google Drive through the Drive API.}]
import json
import mimetypes
